...
BINARY_DISPLAY_NAME="sentinel-git"
SOURCE_BIN="./target/release/sgit"

mkdir -p "$INSTALL_DIR"

if [ -f "$SOURCE_BIN" ]; then
    echo "📦 Copying binary to $INSTALL_DIR..."
    cp "$SOURCE_BIN" "$INSTALL_DIR/$BINARY_DISPLAY_NAME"
    chmod +x "$INSTALL_DIR/$BINARY_DISPLAY_NAME"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        if ! grep -q "alias sg=" "$SHELL_CONFIG"; then
            ...
            echo "alias sg='$BINARY_DISPLAY_NAME'" >> "$SHELL_CONFIG"
        ...
        echo "   alias sg='$BINARY_DISPLAY_NAME'"
    fi

    echo "✅ SentinelGit installed successfully!"
    echo "   Run '$BINARY_DISPLAY_NAME' or 'sg' (after reloading shell) to start."
else
